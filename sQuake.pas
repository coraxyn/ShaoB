...
// fCon.Send( s, taBold );
          if ( StrToFloatDef( mg, 0 ) >= 5.5 ) and ( id <> fLastQuake ) then begin
            ...
            s := 'Earthquake ' + fJSON.FindPath( 'properties.flynn_region' ).AsString +
                 ...
                 ', lat ' + formatFloat( '###0.00', StrToFloatDef( fJSON.FindPath( 'properties.lat' ).AsString, 0 ) ) +
                 ', lon ' + formatFloat( '###0.00', StrToFloatDef( fJSON.FindPath( 'properties.lon' ).AsString, 0 ) );
            fIRC.MsgChat( s );
