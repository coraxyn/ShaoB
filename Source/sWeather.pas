...
implementation
  ...
  function tWeather.Conditions( sCity, sProvince, sCountry : string ) : string;
    // Get weather conditions from OPENWEATHERMAP.ORG
  var
    http : THTTPSend;
    i    : integer;
    jsn  : TJSONData;
  ...
  begin
    ...
    if length( sCity ) > 0 then begin
      ...
      http.Protocol   := '1.1';
      res := 'https://api.apixu.com/v1/current.json?key=' + fAPIXU + '&q=' + sCity;
      if length( sProvince ) > 0 then res := res + ',' + sProvince;
      if length( sCountry ) > 0 then res := res + ',' + sCountry;
      res := EncodeURL( res );
      try
        if http.HTTPMethod( 'GET', res ) then begin
          str.LoadFromStream( http.Document );
          for i := 0 to str.Count - 1 do s := s + str[ i ];
          jsn := GetJson( s );
          res := '';
          s   := '';
          try
            s := jsn.FindPath( 'location.name' ).AsString;
            if length( s ) > 0 then res := res + s + ', ';
            s := jsn.FindPath( 'location.region' ).AsString;
            if length( s ) > 0 then res := res + s + ', ' ;
            s := jsn.FindPath( 'location.country' ).AsString;
            if length( s ) > 0 then res := res + s + ', ';
            s := jsn.FindPath( 'current.condition.text' ).AsString;
            if length( s ) > 0 then res := res + s + ', ';
            s := jsn.FindPath( 'current.temp_c' ).AsString;
            t := s;
            if length( s ) > 0 then res := res + FormatFloat( '#0.0', strToFloat( s ) ) + ' C ';
            s := jsn.FindPath( 'current.feelslike_c' ).AsString;
            if ( length( s ) > 0 ) and ( s <> t ) then res := res + 'as ' + FormatFloat( '#0.0', strToFloat( s ) ) + ' C, ';
            s := jsn.FindPath( 'current.wind_kph' ).AsString;
            if ( length( s ) > 0 ) and ( strToFloatDef( s, 0.0 ) > 0.0 ) then res := res + 'wind ' + FormatFloat( '##0.0', strToFloat( s ) ) + ' kph ';
            s := jsn.FindPath( 'current.wind_dir' ).AsString;
            if length( s ) > 0 then res := res + s + ', ';
            s := jsn.FindPath( 'current.precip_mm' ).AsString;
            if ( length( s ) > 0 ) and ( strToFloatDef( s, 0.0 ) > 0.0 ) then res := res + 'precip ' + FormatFloat( '###0.0', strToFloat( s ) ) + ' mm, ';
            s := jsn.FindPath( 'current.pressure_mb' ).AsString;
            if length( s ) > 0 then res := res + 'pressure ' + FormatFloat( '000.0', strToFloat( s ) / 10 ) + ' kPa, ';
            s := jsn.FindPath( 'current.humidity' ).AsString;
            if length( s ) > 0 then res := res + 'humidity ' + s + '%, ';
            s := jsn.FindPath( 'current.uv' ).AsString;
            if ( length( s ) > 0 ) and ( strToFloatDef( s, 0.0 ) > 0.0 ) then res := res + 'uv ' + FormatFloat( '#0', strToFloat( s ) );
          except
            res := res + ' Oops, JSON failed';
          end;
        end else res := 'Weather failed: ' + http.sock.GetErrorDescEx;
      finally
        http.Free;
        str.Free;
      end;  // try
    end else res := 'At least enter city';
    Conditions := res;
