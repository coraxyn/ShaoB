...
implementation
  ...
  begin
    ...
    t := '';
    try
      curl_easy_setopt( fCurlp, CURLOPT_VERBOSE, [ TRUE ] );                        // Turn verbose off
      ...
      if length( a ) > 0                                                             // Check for any more/custom headers
        then for i := 0 to length( a ) - 1 do begin
                a[ i ] := a[ i ] + #00;
                curl_easy_setopt( fCurlp, CURLOPT_HEADER, [ @a[ i, 1 ] ] );
             end;
