...
implementation
  ...
  type
    tIndex = ( id8Ball, idAbout, idAnagram, idAurora, idDefine, idDoF, idFPC, idHelp, idHost, idInfo, idLaunch, idNote, idOps, idPodcast,
...
//writeln( 'i-', i, ' j-' , j );
      ...
      on E:EJSON do begin
        s := 'Error occured';
        fCon.Send( '.FPC: ' + E.Message + ' ' + E.ClassName, taBold );
      end;
      on E:Exception do begin
        s := 'Error occured';
        fCon.Send( '.FOC: ' + E.Message + ' ' + E.ClassName, taBold );
      end;
    end;  // except
    Result := s;
  end;  // tIRC.FPC
...
//    i : integer;
  begin
    with fIndex do begin
      ...
      CommaText       := '.8BALL,.ABOUT,.ANAGRAM,.AURORA,.DEFINE,.DOF,.FPC,.HELP,.HOST,.INFO,.LAUNCH,.NOTE,.OPS,.PODCAST,.PROFILE,.QUIT,.RAP,.SHAO,.SPACEX,.SUNSPOTS,.SYNONYMS,.TIME,.TOPIC,.UP,.VERSION,.WEATHER,.WIKI,.WIKIFP,' +
                         '/ACTION,/TIME,/VERSION,' +
